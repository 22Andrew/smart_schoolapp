document.addEventListener('DOMContentLoaded', function () {
    ...
    function populateSections(classId) {
        const selected = classes.find(function (item) { return String(item.id) === String(classId); });
        const sections = selected && selected.sections ? selected.sections : [];
        sectionSelect.innerHTML = sections.map(function (section, index) {
            return '<option value="' + escapeHtml(section) + '"' + (index === 0 ? ' selected' : '') + '>'
    ...
    async function loadClasses() {
        ...
        classSelect.innerHTML = classes.map(function (item, index) {
            return '<option value="' + item.id + '"' + (index === 0 ? ' selected' : '') + '>'
                + escapeHtml(item.name) + '</option>';
        }).join('');
        if (classes.length) {
            populateSections(classes[0].id);
        } else {
    ...
    async function loadTemplates() {
        const response = await fetch('/api/cbse-exam-reports/template-options');
        if (!response.ok) throw new Error('Failed to load templates');
        const templates = await response.json();
        templateSelect.innerHTML = templates.map(function (item, index) {
            return '<option value="' + escapeHtml(item.id) + '"' + (index === 0 ? ' selected' : '') + '>'
                + escapeHtml(item.name) + '</option>';
        }).join('');
    }
    ...
    async function searchReport() {
        ...
        if (!classId || !section || !templateId) {
            Swal.fire({ icon: 'warning', title: 'Required Fields', text: 'Please select class, section, and template.', confirmButtonColor: '#8b5cf6' });
            return;
        }

        const response = await fetch('/api/cbse-exam-reports/exam-template?classId=' + encodeURIComponent(classId)
    ...
    document.getElementById('printBtn')?.addEventListener('click', function () {
        ...
        win.document.write('<html><head><title>Template Marks Report</title>'
            ...
            + '<h1>' + escapeHtml(reportData.templateName) + ' - Template Marks Report</h1>'
    ...
    document.getElementById('excelBtn')?.addEventListener('click', function () {
        if (!reportData) return;
        const headers = ['Student', 'Admission No', 'Class'];
        ...
        const rows = (reportData.rows || []).map(function (row) {
            const line = [row.studentName, row.admissionNo, row.className || ''];
            let markIndex = 0;
            reportData.subjects.forEach(function (subject) {
                (subject.assessments || []).forEach(function () {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Template Marks');
        XLSX.writeFile(wb, 'Template_Marks_Report_' + new Date().toISOString().split('T')[0] + '.xlsx');
    });

    Promise.all([loadClasses(), loadTemplates()])
        .then(function () {
            if (classes.length && templateSelect.options.length) {
                return searchReport();
            }
        })
        .catch(showError);
