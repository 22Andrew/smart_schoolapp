...
async function loadDepartments() {
    try {
        ...
        departmentRecords = await response.json();
        filteredRecords = [...departmentRecords];
        currentPage = 1;
        renderTable();
    } catch (error) {
...
function renderTable(records = filteredRecords) {
    ...
    if (paginatedRecords.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="2" style="text-align: center; padding: 40px;">No Departments found</td>
            </tr>
        `;
    } else {
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="action-buttons">
                        <button type="button" class="btn-action" onclick="editDepartment(${record.id})" title="Edit">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <path d="M17 3a2.8 2.8 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"></path>
                            </svg>
                        </button>
                        <button type="button" class="btn-action" onclick="deleteDepartment(${record.id})" title="Delete">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
async function handleFormSubmit(event) {
    ...
    try {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: error.message || 'Failed to save Department',
            confirmButtonText: 'OK',
            confirmButtonColor: '#ef4444'
        });
    }
...
function editDepartment(id) {
    const record = departmentRecords.find(item => item.id === id);
    if (!record) return;

    editingId = id;
    document.getElementById('departmentId').value = id;
    document.getElementById('departmentName').value = record.name;
    document.querySelector('.add-department-panel .panel-title').textContent = 'Edit Department';
...
function resetForm() {
    ...
    document.querySelector('.add-department-panel .panel-title').textContent = 'Add Department';
}

function handleSearch(event) {
    const searchTerm = event.target.value.toLowerCase();
    filteredRecords = departmentRecords.filter(record =>
