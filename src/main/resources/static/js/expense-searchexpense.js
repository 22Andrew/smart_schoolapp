document.addEventListener('DOMContentLoaded', function () {
    ...
    const tableBody = document.getElementById('expenseSearchTableBody');
    const tableSearchInput = document.getElementById('tableSearchInput');
    const entriesSelect = document.getElementById('entriesSelect');
    const showingInfo = document.getElementById('showingInfo');
    const pagination = document.getElementById('pagination');
    const expenseSearchTable = document.getElementById('expenseSearchTable');
    ...
    async function fetchExpenses(searchTypeValue, keyword) {
        const params = new URLSearchParams();
        if (searchTypeValue) params.set('searchType', searchTypeValue);
        if (keyword) params.set('keyword', keyword);

        const response = await fetch('/api/expenses/search?' + params.toString());
        if (!response.ok) {
            const err = await response.json().catch(function () { return {}; });
            throw new Error(err.message || 'Failed to load expense records');
        }
    ...
    async function searchByType() {
        try {
            expenseRows = await fetchExpenses(searchType.value, '');
            currentPage = 1;
            renderTable();
        } catch (error) {
            showError(error);
    ...
    async function searchByKeyword() {
        ...
        try {
            expenseRows = await fetchExpenses('all', keyword);
            currentPage = 1;
            renderTable();
        } catch (error) {
            showError(error);
    ...
    function sortValue(row, key) {
        switch (key) {
            ...
            case 'expenseHead': return row.expenseHead || '';
    ...
    function renderTable() {
        ...
        if (!total) {
            tableBody.innerHTML = ''
                ...
                + '<div class="empty-state"><p class="empty-message">No expense records found</p></div>'
                + '</td></tr>';
        ...
        tableBody.innerHTML = pageRows.map(function (row) {
            return '<tr>'
                ...
                + '<td>' + escapeHtml(row.expenseHead || '') + '</td>'
    ...
    if (expenseSearchTable) {
        expenseSearchTable.querySelectorAll('th[data-sort]').forEach(function (th) {
    ...
    document.getElementById('excelBtn')?.addEventListener('click', function () {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Expense Search');
        XLSX.writeFile(wb, 'expense-search.xlsx');
    });
    ...
    document.getElementById('csvBtn')?.addEventListener('click', function () {
        ...
        link.download = 'expense-search.csv';
        link.click();
    });
    ...
    document.getElementById('pdfBtn')?.addEventListener('click', function () {
        ...
        doc.text('Expense Search Report', 40, 40);
        doc.autoTable({
            startY: 60,
            head: [['Name', 'Invoice Number', 'Expense Head', 'Date', 'Amount ($)']],
            body: rows.map(function (row) {
                return [row.Name, row['Invoice Number'], row['Expense Head'], row.Date, row['Amount ($)']];
            })
        });
        doc.save('expense-search.pdf');
    });
