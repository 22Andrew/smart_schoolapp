document.addEventListener('DOMContentLoaded', function () {
    ...
    const searchForm = document.getElementById('timetableSearchForm');
    ...
    let classes = [];
    let currentTitle = 'Class Timetable';
    let periodsByDay = {};
    ...
    function periodCardHtml(period) {
        ...
        return ''
            ...
            + '<div class="period-line">' + iconHtml('book') + '<span>Subject: ' + escapeHtml(subjectDisplay(period)) + '</span></div>'
            + '<div class="period-line">' + iconHtml('clock') + '<span>' + escapeHtml(timeRange) + '</span></div>'
            + '<div class="period-line">' + iconHtml('user') + '<span>' + escapeHtml(period.teacherName || '-') + '</span></div>'
            + '<div class="period-line">' + iconHtml('room') + '<span>Room No.: ' + escapeHtml(period.roomNo || '-') + '</span></div>'
            + '</div>';
    }
    ...
    searchForm.addEventListener('submit', async function (e) {
        ...
        currentTitle = className + ' (' + sectionSelect.value + ') Timetable';
        document.querySelector('.timetable-toolbar .panel-title').textContent = currentTitle;
        ...
        try {
            ...
            Swal.fire({
                ...
                text: entries.length
                    ? 'Showing timetable for ' + className + ' (' + sectionSelect.value + ').'
                    : 'No timetable saved yet for ' + className + ' (' + sectionSelect.value + '). Use Add to create one.',
        ...
        printBtn.addEventListener('click', function () {
            if (timetablePanel.style.display === 'none') {
                Swal.fire({
                    icon: 'warning',
                    title: 'No Timetable',
                    text: 'Please search for a class timetable first.',
                    confirmButtonColor: '#8b5cf6'
                });
                return;
            }
            ...
            printWindow.document.write(''
                + '<!DOCTYPE html><html><head><title>' + currentTitle + '</title><style>'
                ...
                + '.body{padding:8px}'
                + '.card{border:1px solid #ddd;border-radius:4px;padding:8px;margin-bottom:8px;font-size:12px}'
                    ...
                    if (!dayPeriods.length) {
                        ...
                        body = dayPeriods.map(function (p) {
                            return '<div class="card"><div>Subject: ' + escapeHtml(subjectDisplay(p))
                                ...
                                + '</div><div>' + escapeHtml(p.teacherName || '-')
