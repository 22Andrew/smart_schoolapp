...
async function loadRecords() {
    try {
        const response = await fetch('/api/communicate/messages?status=SCHEDULED');
        ...
        Swal.fire({ icon: 'error', title: 'Error', text: error.message || 'Failed to load scheduled logs' });
    }
}
...
function renderTable() {
    ...
    if (!pageRecords.length) {
        tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:40px;">No scheduled messages found</td></tr>';
    } else {
        pageRecords.forEach(record => {
            const row = document.createElement('tr');
            const messageType = String(record.messageType || '').toUpperCase();
            const recipientType = String(record.recipientType || '');
            const displayDate = formatDisplayDate(record.createdAt || record.sentAt);
            ...
            row.innerHTML = `
                ...
                <td class="col-check">${isGroupRecipient(recipientType) ? `<span class="log-check">${CHECK_ICON}</span>` : ''}</td>
                ...
                <td>
                    <div class="schedule-action-buttons">
                        <button type="button" class="btn-view-log" onclick="viewRecord(${record.id})" title="View">
                        ...
                        <button type="button" class="btn-delete-log-row" onclick="deleteRecord(${record.id})" title="Delete">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
function isGroupRecipient(recipientType) {
    return recipientType === 'Group' || ['Student', 'Guardian', 'Staff', 'All'].includes(recipientType);
}
    ...
    Swal.fire({
        ...
        html: `
            <div class="schedule-view-details">
                <p><strong>Type:</strong> ${escapeHtml(messageType)}</p>
                <p><strong>Date:</strong> ${escapeHtml(formatDisplayDate(record.createdAt || record.sentAt))}</p>
                <p><strong>Schedule Date:</strong> ${escapeHtml(formatDisplayDate(record.scheduledAt))}</p>
                <p><strong>Recipient:</strong> ${escapeHtml(recipientType)}${record.recipientDetails ? ` (${escapeHtml(record.recipientDetails)})` : ''}</p>
                <p><strong>Message:</strong></p>
                <div class="schedule-view-message">${escapeHtml(record.message || '')}</div>
            </div>
        `,
        width: 640,
...
async function deleteRecord(id) {
    const confirmed = await Swal.fire({
        ...
        text: 'This scheduled message will be permanently removed.',
        icon: 'warning',
...
function exportRows() {
    return filteredRecords.map(record => ({
        ...
        Date: formatDisplayDate(record.createdAt || record.sentAt),
        ...
        Group: isGroupRecipient(String(record.recipientType || '')) ? 'Yes' : '',
...
function handleExcelExport() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Schedule Log');
    XLSX.writeFile(wb, 'schedule-email-sms-log.xlsx');
}
...
function handleCSVExport() {
    ...
    link.href = url;
    link.download = 'schedule-email-sms-log.csv';
    link.click();
...
function handlePDFExport() {
    ...
    const body = exportRows().map(row => [
        row.Title, row.Message, row.Date, row['Schedule Date'],
    ...
    doc.save('schedule-email-sms-log.pdf');
}
