document.addEventListener('DOMContentLoaded', function () {
    ...
    function applySearchFilter() {
        ...
        Array.from(tableBody.querySelectorAll('tr')).forEach(function (row) {
            ...
            const rowText = Array.from(row.querySelectorAll('td'))
                .filter(function (cell, index) { return index < 1; })
    ...
    function renderCategories() {
        ...
        if (!categories.length) {
            tableBody.innerHTML = '<tr class="no-data-row"><td colspan="2" style="text-align:center;color:#94a3b8;">No categories found</td></tr>';
            updateShowingInfo(0, '');
            return;
        }
    ...
    async function loadCategories() {
        try {
            const response = await fetch('/api/cbse-exam-categories');
            if (!response.ok) {
    ...
    if (categoryForm) {
        categoryForm.addEventListener('submit', async function (e) {
            ...
            try {
                let response;
                if (editingId) {
                    response = await fetch('/api/cbse-exam-categories/' + editingId, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(payload)
                    });
                } else {
                    response = await fetch('/api/cbse-exam-categories', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(payload)
                    });
    ...
    if (tableBody) {
        tableBody.addEventListener('click', function (e) {
            ...
            if (editBtn) {
                categoryIdInput.value = row.getAttribute('data-id') || '';
                categoryNameInput.value = row.querySelector('.category-name').textContent.trim();
            ...
            if (deleteBtn) {
                ...
                Swal.fire({
                    ...
                    try {
                        const response = await fetch('/api/cbse-exam-categories/' + rowId, {
                            method: 'DELETE'
                        });
                        if (!response.ok && response.status !== 204) {
                            throw new Error('Failed to delete category');
                        }
                        if (categoryIdInput.value === rowId) {
                            resetForm();
                        }
                        await loadCategories();
                        Swal.fire({
    ...
    if (excelBtn) {
        excelBtn.addEventListener('click', function () {
            ...
            XLSX.writeFile(wb, 'Categories_' + timestamp + '.xlsx');
            Swal.fire({
                icon: 'success',
                title: 'Exported!',
    ...
    if (pdfBtn) {
        pdfBtn.addEventListener('click', function () {
            ...
            const timestamp = new Date().toISOString().split('T')[0];
            doc.save('Categories_' + timestamp + '.pdf');
            Swal.fire({
                icon: 'success',
                title: 'Exported!',
