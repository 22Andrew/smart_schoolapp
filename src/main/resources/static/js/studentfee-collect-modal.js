(function () {
    const sfCollectModal = document.getElementById('sfCollectModal');
    const sfCollectOverlay = document.getElementById('sfCollectOverlay');
    const sfCollectBackBtn = document.getElementById('sfCollectBackBtn');
    const studentIdInput = document.getElementById('studentId');
    const sessionYearInput = document.getElementById('sessionYear');
    const tableBody = document.getElementById('sfStudentFeesTableBody');
    const selectAll = document.getElementById('sfSelectAllFees');
    const collectBtn = document.getElementById('sfCollectSelectedBtn');
    const printSelectedBtn = document.getElementById('sfPrintSelectedBtn');
    ...
    if (!sfCollectModal || !tableBody) return;
    ...
    function statusClass(status) {
        if (status === 'Paid') return 'sf-status-paid';
        if (status === 'Partial') return 'sf-status-partial';
        return 'sf-status-unpaid';
    }
    ...
    function actionButtons(item) {
        return ''
            ...
            + '<button type="button" class="sf-btn-mini btn-collect-one" data-id="' + escapeHtml(String(item.feeMasterId)) + '" title="Add Fee">'
            + '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>'
            + '</button>'
            + '<button type="button" class="sf-btn-mini btn-print-one" title="Print">'
    ...
    function paymentActionButtons(paymentId) {
        return ''
            + '<div class="sf-action-btns">'
            + '<button type="button" class="sf-btn-mini btn-reverse" data-payment-id="' + escapeHtml(String(paymentId)) + '" title="Reverse Payment">'
    ...
    function fillStudent(student) {
        document.getElementById('sfStudentName').textContent = displayOrDash(student.name) || '-';
        document.getElementById('sfFatherName').textContent = displayOrDash(student.fatherName) || '-';
        document.getElementById('sfMobileNumber').textContent = displayOrDash(student.mobileNumber) || '-';
        document.getElementById('sfCategoryName').textContent = displayOrDash(student.categoryName) || '-';
        document.getElementById('sfClassSection').textContent = displayOrDash(student.classSection) || '-';
        document.getElementById('sfAdmissionNo').textContent = displayOrDash(student.admissionNo) || '-';
        document.getElementById('sfRollNumber').textContent = displayOrDash(student.rollNumber) || '-';
        document.getElementById('sfRteValue').textContent = displayOrDash(student.rte || 'No');

        const photo = document.getElementById('sfStudentPhoto');
        const placeholder = document.getElementById('sfPhotoPlaceholder');
        if (student.photoUrl) {
    ...
    function renderFees() {
        ...
        fees.forEach(function (item) {
            ...
            html += '<tr class="' + rowClass(item.status) + '" data-fee-master-id="' + escapeHtml(String(item.feeMasterId)) + '">'
                ...
                + '<td><span class="sf-status-badge ' + statusClass(item.status) + '">' + escapeHtml(item.status) + '</span></td>'
    ...
    async function loadPage() {
        ...
        document.getElementById('sfSessionLabel').textContent = data.sessionYear || sessionYear;
        document.getElementById('sfDateLabel').textContent = formatDate(data.date) || todayDisplay();
        if (data.sessionYear) sessionYear = data.sessionYear;
    ...
    function openModal(id) {
        ...
        sfCollectModal.classList.add('active');
        sfCollectModal.setAttribute('aria-hidden', 'false');
    ...
    function closeModal() {
        sfCollectModal.classList.remove('active');
        sfCollectModal.setAttribute('aria-hidden', 'true');
    ...
    function closeCollectSelectedModal() {
        if (!collectSelectedModal) return;
    ...
    if (sfCollectOverlay) sfCollectOverlay.addEventListener('click', closeModal);
    if (sfCollectBackBtn) sfCollectBackBtn.addEventListener('click', closeModal);
    ...
    document.addEventListener('keydown', function (e) {
        ...
        if (collectSelectedModal && collectSelectedModal.classList.contains('active')) {
            closeCollectSelectedModal();
            return;
        }
        if (addFeeModal && addFeeModal.classList.contains('active')) {
            closeAddFeeModal();
            return;
        }
        if (sfCollectModal.classList.contains('active')) {
            closeModal();
        }
    });

    ['sfCopyBtn', 'sfExcelBtn', 'sfCsvBtn', 'sfPdfBtn', 'sfPrintBtn'].forEach(function (id) {
