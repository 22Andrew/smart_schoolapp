document.addEventListener('DOMContentLoaded', function () {
    const categoryForm = document.getElementById('itemCategoryForm');
    ...
    const table = document.getElementById('itemCategoryTable');
    const tableBody = document.getElementById('itemCategoryTableBody');
    ...
    function filteredRows() {
        ...
        return rows.filter(function (row) {
            return [row.name, row.description].some(function (value) {
    ...
    async function loadCategories() {
        try {
            ...
            if (!response.ok) throw new Error('Failed to load item categories');
        ...
        categoryForm.addEventListener('submit', async function (e) {
            ...
            if (!payload.name) {
                Swal.fire({
                    ...
                    text: 'Item Category is required.',
            ...
            const url = id
                ? '/api/inventory/item-categories/' + encodeURIComponent(id)
            ...
            try {
                ...
                if (!response.ok || data.success === false) {
                    throw new Error(data.message || 'Failed to save item category');
                }
                resetForm();
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Saved',
                    text: data.message || 'Item category saved successfully!',
                    timer: 1600,
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: error.message || 'Failed to save item category.',
                    confirmButtonColor: '#8b5cf6'
                });
            }
        });
    ...
    if (tableBody) {
        tableBody.addEventListener('click', async function (e) {
            ...
            const result = await Swal.fire({
                ...
                text: 'This category will be removed from inventory.',
            ...
            try {
                const response = await fetch('/api/inventory/item-categories/' + encodeURIComponent(id), { method: 'DELETE' });
                const data = await response.json().catch(function () { return {}; });
                if (!response.ok || data.success === false) {
                    throw new Error(data.message || 'Failed to delete item category');
                }
                if (categoryIdInput.value === String(id)) resetForm();
                await loadCategories();
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: error.message || 'Failed to delete item category.',
                    confirmButtonColor: '#8b5cf6'
                });
            }
        });
    ...
    ['copyBtn', 'excelBtn', 'csvBtn', 'pdfBtn', 'printBtn'].forEach(function (id) {
        ...
        btn.addEventListener('click', function () {
            ...
            downloadCsv(id === 'excelBtn' ? 'item-category-list.xls' : 'item-category-list.csv');
        });
    });
