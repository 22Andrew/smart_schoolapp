document.addEventListener('DOMContentLoaded', function () {
    ...
    function applySearchFilter() {
        ...
        Array.from(tableBody.querySelectorAll('tr')).forEach(function (row) {
            ...
            const rowText = (row.querySelector('.section-name') || {}).textContent || '';
    ...
    function renderSections() {
        ...
        if (!sections.length) {
            tableBody.innerHTML = '<tr class="no-data-row"><td colspan="2" style="text-align:center;color:#94a3b8;">No sections found</td></tr>';
            updateShowingInfo(0, '');
            return;
        }
        ...
        sections.forEach(function (item) {
            ...
            tr.innerHTML = ''
                + '<td class="section-name">' + escapeHtml(item.sectionName) + '</td>'
    ...
    async function loadSections() {
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to load sections from database.',
    ...
    if (sectionForm) {
        sectionForm.addEventListener('submit', async function (e) {
            ...
            const editingId = sectionIdInput.value;
            const payload = { sectionName: sectionName };

            try {
                let response;
                if (editingId) {
                    response = await fetch('/api/sections/' + editingId, {
                        method: 'PUT',
                ...
                Swal.fire({
                    ...
                    text: editingId ? 'Section updated successfully.' : 'Section saved to database.',
                    timer: 1500,
    ...
    if (tableBody) {
        tableBody.addEventListener('click', function (e) {
            ...
            if (editBtn) {
                sectionIdInput.value = row.getAttribute('data-id') || '';
                sectionNameInput.value = row.querySelector('.section-name').textContent.trim();
            ...
            if (deleteBtn) {
                const name = row.querySelector('.section-name').textContent.trim();
                ...
                Swal.fire({
                    ...
                    try {
                        const response = await fetch('/api/sections/' + rowId, {
                            method: 'DELETE'
                        });
                        if (!response.ok && response.status !== 204) {
                            throw new Error('Failed to delete section');
                        }
                        if (sectionIdInput.value === rowId) {
                            resetForm();
                        }
                        await loadSections();
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted',
                            text: 'Section deleted from database.',
                            timer: 1400,
                            showConfirmButton: false
                        });
                    } catch (error) {
    ...
    if (excelBtn) {
        excelBtn.addEventListener('click', function () {
            ...
            XLSX.writeFile(wb, 'Sections_' + timestamp + '.xlsx');
            Swal.fire({
                icon: 'success',
                title: 'Exported!',
    ...
    if (csvBtn) {
        csvBtn.addEventListener('click', function () {
            ...
            link.setAttribute('download', 'Sections_' + timestamp + '.csv');
    ...
    if (pdfBtn) {
        pdfBtn.addEventListener('click', function () {
            ...
            const timestamp = new Date().toISOString().split('T')[0];
            doc.save('Sections_' + timestamp + '.pdf');
            Swal.fire({
                icon: 'success',
                title: 'Exported!',
    ...
    if (printBtn) {
        printBtn.addEventListener('click', function () {
            const result = getTableData();
            let printContent = ''
                + '<!DOCTYPE html><html><head><title>Section List - Print</title><style>'
