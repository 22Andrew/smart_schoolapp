document.addEventListener('DOMContentLoaded', function () {
    const table = document.getElementById('expenseHeadTable');
    const tableBody = document.getElementById('expenseHeadTableBody');
    const showingInfo = document.querySelector('.showing-info');
    const searchInput = document.getElementById('searchInput');
    const expenseHeadForm = document.getElementById('expenseHeadForm');
    const expenseHeadNameInput = document.getElementById('expenseHeadName');
    const expenseHeadDescriptionInput = document.getElementById('expenseHeadDescription');
    const expenseHeadIdInput = document.getElementById('expenseHeadId');
    ...
    function resetForm() {
        if (!expenseHeadForm) return;
        expenseHeadForm.reset();
        expenseHeadIdInput.value = '';
    ...
    function renderHeads() {
        ...
        if (!total) {
            tableBody.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:40px;color:#94a3b8;">No expense heads found</td></tr>';
    ...
    async function loadHeads() {
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to load expense heads.',
    ...
    if (expenseHeadForm) {
        expenseHeadForm.addEventListener('submit', async function (e) {
            e.preventDefault();
            const name = expenseHeadNameInput.value.trim();
            const description = expenseHeadDescriptionInput.value.trim();

            if (!name) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Required',
                    text: 'Please enter an expense head.',
                    confirmButtonColor: '#8b5cf6'
                });
                return;
            }

            const editingId = expenseHeadIdInput.value;
            ...
            try {
                ...
                if (!response.ok || !result.success) {
                    throw new Error(result.message || 'Failed to save expense head');
                }
                ...
                Swal.fire({
                    ...
                    text: result.message || 'Expense head saved successfully.',
                    timer: 1500,
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: error.message || 'Failed to save expense head.',
                    confirmButtonColor: '#8b5cf6'
                });
            }
        });
    ...
    if (tableBody) {
        tableBody.addEventListener('click', function (e) {
            ...
            if (editBtn) {
                expenseHeadIdInput.value = row.getAttribute('data-id') || '';
                expenseHeadNameInput.value = row.querySelector('.head-name').textContent.trim();
                expenseHeadDescriptionInput.value = row.querySelector('.head-description').textContent.trim();
                saveBtn.textContent = 'Update';
                expenseHeadNameInput.focus();
                return;
            }
            ...
            if (deleteBtn) {
                ...
                Swal.fire({
                    ...
                    try {
                        const response = await fetch('/api/expense-heads/' + rowId, { method: 'DELETE' });
                        const data = await response.json().catch(function () { return {}; });
                        if (!response.ok || !data.success) {
                            throw new Error(data.message || 'Failed to delete expense head');
                        }
                        if (expenseHeadIdInput.value === rowId) resetForm();
                        await loadHeads();
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted',
                            text: 'Expense head deleted successfully.',
                            timer: 1400,
                            showConfirmButton: false
                        });
                    } catch (error) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: error.message || 'Failed to delete expense head.',
                            confirmButtonColor: '#8b5cf6'
                        });
                    }
                });
    ...
    document.getElementById('copyBtn')?.addEventListener('click', function () {
        ...
        Swal.fire({ icon: 'success', title: 'Copied!', text: 'Expense head list copied to clipboard', timer: 1500, showConfirmButton: false });
    });
    ...
    document.getElementById('pdfBtn')?.addEventListener('click', function () {
        ...
        doc.text('Expense Head List', 14, 16);
        doc.autoTable({
            startY: 24,
            head: [['Expense Head', 'Description']],
            body: rows.map(function (row) {
                return [row['Expense Head'], row.Description];
            })
        });
