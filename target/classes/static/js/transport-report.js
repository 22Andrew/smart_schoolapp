document.addEventListener('DOMContentLoaded', function () {
    const root = document.getElementById('trReportRoot');
    if (!root) return;

    const reportKey = root.dataset.reportKey || 'studenttransportdetails';
    const apiUrl = root.dataset.apiUrl || '';
    const listTitle = root.dataset.listTitle || 'Student Transport Report';

    const criteriaForm = document.getElementById('trCriteriaForm');
    ...
    const columns = [
        ...
        { key: 'routeTitle', label: 'Route Title' },
    ...
    async function loadLookups() {
        const [classesRes, sectionsRes, optionsRes] = await Promise.all([
            fetch('/api/classes'),
            fetch('/api/sections'),
            fetch('/api/transport/options')
        ]);

        if (!classesRes.ok || !sectionsRes.ok || !optionsRes.ok) {
            throw new Error('Failed to load transport report filters');
        }
        ...
        fillNamedSelect(routeSelect, options.routes || [], 'title', 'title');
        fillNamedSelect(pickupSelect, options.pickupPoints || [], 'name', 'name');
        fillNamedSelect(vehicleSelect, options.vehicles || [], 'vehicleNumber', 'vehicleNumber');
    }

    async function loadReport() {
        const params = new URLSearchParams();
        if (classSelect.value) params.set('classId', classSelect.value);
        if (sectionSelect.value) params.set('section', sectionSelect.value);
        if (routeSelect.value) params.set('source', routeSelect.value);
        if (pickupSelect.value) params.set('role', pickupSelect.value);
        if (vehicleSelect.value) params.set('designation', vehicleSelect.value);
