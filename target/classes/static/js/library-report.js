document.addEventListener('DOMContentLoaded', function () {
    const root = document.getElementById('libReportRoot');
    if (!root) return;

    const reportKey = root.dataset.reportKey || 'studentbookissuereport';
    const apiUrl = root.dataset.apiUrl || '';
    const listTitle = root.dataset.listTitle || 'Book Issue Report';
    const showMemberType = root.dataset.showMemberType === 'true';

    const criteriaForm = document.getElementById('libCriteriaForm');
    ...
    const defaultPageSize = (reportKey === 'bookinventory' || reportKey === 'bookduereport') ? 100 : 50;
    let pageSize = defaultPageSize;

    const COLUMN_SETS = {
        studentbookissuereport: [
            ...
            { key: 'membersType', label: 'Members Type' }
        ],
        bookduereport: [
            { key: 'bookTitle', label: 'Book Title' },
            { key: 'bookNumber', label: 'Book Number' },
            ...
            { key: 'dueReturnDate', label: 'Due Return Date' },
            { key: 'memberId', label: 'Member ID' },
            { key: 'libraryCardNumber', label: 'Library Card No.' },
            { key: 'admissionNo', label: 'Admission No' },
            { key: 'issueBy', label: 'Issue By' },
            { key: 'membersType', label: 'Members Type' }
        ],
    ...
    columns = COLUMN_SETS[reportKey] || COLUMN_SETS.studentbookissuereport;
    if (resultsHeading) {
    ...
    async function loadReport() {
        ...
        if (showMemberType && memberTypeSelect && memberTypeSelect.value) {
            params.set('source', memberTypeSelect.value);
        }
