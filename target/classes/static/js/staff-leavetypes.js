...
function setupEventListeners() {
    document.getElementById('leaveTypeForm')?.addEventListener('submit', handleFormSubmit);
...
async function loadLeaveTypes() {
    try {
        const response = await fetch('/api/staff-leave-types');
        if (!response.ok) throw new Error('Failed to fetch leave types');

        leaveTypeRecords = await response.json();
        filteredRecords = [...leaveTypeRecords];
        currentPage = 1;
        renderTable();
    } catch (error) {
...
function renderTable(records = filteredRecords) {
    ...
    if (paginatedRecords.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="2" style="text-align: center; padding: 40px;">No leave types found</td>
            </tr>
        `;
    } else {
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="action-buttons">
                        <button type="button" class="btn-action" onclick="editLeaveType(${record.id})" title="Edit">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <path d="M17 3a2.8 2.8 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"></path>
                            </svg>
                        </button>
                        <button type="button" class="btn-action" onclick="deleteLeaveType(${record.id})" title="Delete">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
async function handleFormSubmit(event) {
    ...
    const payload = { name };
    const url = editingId ? `/api/staff-leave-types/${editingId}` : '/api/staff-leave-types';
    ...
    try {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: error.message || 'Failed to save leave type',
            confirmButtonText: 'OK',
            confirmButtonColor: '#ef4444'
        });
    }
...
function editLeaveType(id) {
    const record = leaveTypeRecords.find(item => item.id === id);
    if (!record) return;

    editingId = id;
    document.getElementById('leaveTypeId').value = id;
    document.getElementById('leaveTypeName').value = record.name;
    document.querySelector('.add-leavetype-panel .panel-title').textContent = 'Edit Leave Type';
...
async function deleteLeaveType(id) {
    ...
    try {
        const response = await fetch(`/api/staff-leave-types/${id}`, { method: 'DELETE' });
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: error.message || 'Failed to delete leave type',
            confirmButtonText: 'OK',
            confirmButtonColor: '#ef4444'
        });
    }
...
function resetForm() {
    ...
    document.querySelector('.add-leavetype-panel .panel-title').textContent = 'Add Leave Type';
}

function handleSearch(event) {
    const searchTerm = event.target.value.toLowerCase();
    filteredRecords = leaveTypeRecords.filter(record =>
...
function handleExcelExport() {
    const data = filteredRecords.map(record => ({ Name: record.name }));
...
function handleCSVExport() {
    const csvData = [['Name'], ...filteredRecords.map(record => [record.name])];
