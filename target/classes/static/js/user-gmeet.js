(function () {
    ...
    var tableBody = document.getElementById('ugmTableBody');
    var table = document.getElementById('ugmLiveClassesTable');
    var searchInput = document.getElementById('ugmSearchInput');
    var entriesSelect = document.getElementById('ugmEntriesSelect');
    var showingInfo = document.getElementById('ugmShowingInfo');
    var pagination = document.getElementById('ugmPagination');
    var columnsBtn = document.getElementById('ugmColumnsBtn');
    var columnsDropdown = document.getElementById('ugmColumnsDropdown');
    ...
    function statusClass(status) {
        ...
        if (value === 'started') return 'status-started';
    ...
    function bindEvents() {
        ...
        ['ugmCopyBtn', 'ugmExcelBtn', 'ugmCsvBtn', 'ugmPdfBtn', 'ugmPrintBtn'].forEach(function (id) {
            var btn = document.getElementById(id);
            if (!btn) return;
            btn.addEventListener('click', function () {
                var type = 'copy';
                if (id === 'ugmExcelBtn') type = 'excel';
                else if (id === 'ugmCsvBtn') type = 'csv';
                else if (id === 'ugmPdfBtn') type = 'pdf';
                else if (id === 'ugmPrintBtn') type = 'print';
                exportTable(type);
            });
        });
        ...
        if (columnsBtn && columnsDropdown) {
            ...
            columnsDropdown.querySelectorAll('.ugm-column-toggle').forEach(function (checkbox) {
    ...
    async function loadLiveClasses() {
        try {
            var response = await fetch('/api/user/gmeet');
            if (!response.ok) {
                var err = {};
