...
function setupEventListeners() {
    ...
    document.querySelectorAll('#captchaTable thead th.sortable').forEach(function (th) {
        th.addEventListener('click', function () {
            ...
            document.querySelectorAll('#captchaTable thead th.sortable').forEach(function (header) {
...
async function loadCaptchaSettings() {
    try {
        const response = await fetch('/api/captcha-settings');
        if (!response.ok) throw new Error('Failed to fetch captcha settings');
        captchaRecords = await response.json();
        applyFilters();
    } catch (error) {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: error.message || 'Failed to load captcha settings',
...
function applyFilters() {
    ...
    filteredRecords = captchaRecords.filter(function (record) {
...
function renderTable() {
    ...
    if (records.length === 0) {
        tbody.innerHTML = '<tr><td colspan="2" style="text-align:center;padding:40px;">No captcha settings found</td></tr>';
    } else {
        tbody.innerHTML = records.map(function (record) {
            ...
            return '<tr>'
                ...
                + '<input type="checkbox" data-slug="' + slug + '" ' + checked + ' onchange="toggleCaptchaStatus(this)">'
...
async function toggleCaptchaStatus(checkbox) {
    ...
    try {
        const response = await fetch('/api/captcha-settings/status', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: slug, status: enabled ? 1 : 0, enabled: enabled })
        });
        const data = await response.json();
        if (!data.success) throw new Error(data.message);

        const record = captchaRecords.find(function (item) { return item.slug === slug; });
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: error.message || 'Failed to update captcha status',
            confirmButtonColor: '#ef4444'
        });
    }
}
...
function handleExcelExport() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Captcha Setting');
    XLSX.writeFile(wb, 'Captcha-Setting.xlsx');
}
...
function handleCSVExport() {
    ...
    a.href = url;
    a.download = 'Captcha-Setting.csv';
    a.click();
