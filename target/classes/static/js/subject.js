document.addEventListener('DOMContentLoaded', function () {
    ...
    function getSelectedType() {
        const checked = document.querySelector('input[name="subjectType"]:checked');
        return checked ? checked.value : 'THEORY';
    }

    function setSelectedType(type) {
        const value = (type || 'THEORY').toUpperCase();
        const radio = document.querySelector('input[name="subjectType"][value="' + value + '"]');
        if (radio) {
            radio.checked = true;
        } else {
            document.getElementById('typeTheory').checked = true;
        }
    }
    ...
    function renderSubjects() {
        ...
        if (!subjects.length) {
            tableBody.innerHTML = '<tr class="no-data-row"><td colspan="4" style="text-align:center;color:#94a3b8;">No subjects found</td></tr>';
            updateShowingInfo(0, '');
            return;
        }
        ...
        subjects.forEach(function (item) {
            ...
            tr.innerHTML = ''
                + '<td class="subject-name">' + escapeHtml(item.name) + '</td>'
                + '<td class="subject-code">' + escapeHtml(item.subjectCode || '') + '</td>'
                + '<td class="subject-type">' + escapeHtml(formatType(item.subjectType)) + '</td>'
    ...
    async function loadSubjects() {
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to load subjects from database.',
    ...
    if (subjectForm) {
        subjectForm.addEventListener('submit', async function (e) {
            ...
            const subjectType = getSelectedType();
            ...
            const editingId = subjectIdInput.value;
            const payload = {
                name: name,
            ...
            try {
                let response;
                if (editingId) {
                    response = await fetch('/api/subjects/' + editingId, {
                        method: 'PUT',
                ...
                Swal.fire({
                    ...
                    text: editingId ? 'Subject updated successfully.' : 'Subject saved to database.',
                    timer: 1500,
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: error.message || 'Failed to save subject.',
                    confirmButtonColor: '#8b5cf6'
                });
            }
        });
    ...
    if (tableBody) {
        tableBody.addEventListener('click', function (e) {
            ...
            if (editBtn) {
                ...
                const item = subjects.find(function (s) { return String(s.id) === String(id); });
                if (!item) return;

                subjectIdInput.value = String(item.id);
                subjectNameInput.value = item.name || '';
                subjectCodeInput.value = item.subjectCode || '';
                setSelectedType(item.subjectType || 'THEORY');
            ...
            if (deleteBtn) {
                ...
                Swal.fire({
                    ...
                    try {
                        const response = await fetch('/api/subjects/' + rowId, {
                            method: 'DELETE'
                        });
                        if (!response.ok && response.status !== 204) {
                            throw new Error('Failed to delete subject');
                        }
                        if (subjectIdInput.value === rowId) {
                            resetForm();
                        }
                        await loadSubjects();
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted',
                            text: 'Subject deleted from database.',
                            timer: 1400,
                            showConfirmButton: false
                        });
                    } catch (error) {
    ...
    if (excelBtn) {
        excelBtn.addEventListener('click', function () {
            ...
            XLSX.writeFile(wb, 'Subjects_' + timestamp + '.xlsx');
            Swal.fire({
                icon: 'success',
                title: 'Exported!',
    ...
    if (pdfBtn) {
        pdfBtn.addEventListener('click', function () {
            ...
            const timestamp = new Date().toISOString().split('T')[0];
            doc.save('Subjects_' + timestamp + '.pdf');
            Swal.fire({
                icon: 'success',
                title: 'Exported!',
    ...
    if (printBtn) {
        printBtn.addEventListener('click', function () {
            const result = getTableData();
            let printContent = ''
                + '<!DOCTYPE html><html><head><title>Subject List - Print</title><style>'
