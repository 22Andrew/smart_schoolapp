document.addEventListener('DOMContentLoaded', function () {
    ...
    function applySearchFilter() {
        ...
        Array.from(tableBody.querySelectorAll('tr')).forEach(function (row) {
            ...
            const rowText = Array.from(row.querySelectorAll('td'))
                .filter(function (cell, index) { return index < 3; })
    ...
    function renderHouses() {
        ...
        if (!houses.length) {
            tableBody.innerHTML = '<tr class="no-data-row"><td colspan="4" style="text-align:center;color:#94a3b8;">No school houses found</td></tr>';
            updateShowingInfo(0, '');
            return;
        }
        ...
        houses.forEach(function (item) {
            ...
            tr.innerHTML = ''
                + '<td class="house-name">' + escapeHtml(item.name) + '</td>'
                + '<td class="house-description">' + escapeHtml(item.description || '') + '</td>'
                + '<td class="house-id">' + item.id + '</td>'
    ...
    async function loadHouses() {
        try {
            ...
            if (!response.ok) {
                throw new Error('Failed to load school houses');
            }
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to load school houses from database.',
    ...
    if (houseForm) {
        houseForm.addEventListener('submit', async function (e) {
            ...
            const editingId = houseIdInput.value;
            const payload = { name: name, description: description };

            try {
                let response;
                if (editingId) {
                    response = await fetch('/api/school-houses/' + editingId, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(payload)
                    });
                } else {
                    response = await fetch('/api/school-houses', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(payload)
                    });
                ...
                Swal.fire({
                    ...
                    text: editingId ? 'School house updated successfully.' : 'School house saved to database.',
                    timer: 1500,
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: error.message || 'Failed to save school house.',
                    confirmButtonColor: '#8b5cf6'
                });
            }
        });
    ...
    if (tableBody) {
        tableBody.addEventListener('click', function (e) {
            ...
            if (editBtn) {
                houseIdInput.value = row.getAttribute('data-id') || '';
                houseNameInput.value = row.querySelector('.house-name').textContent.trim();
                houseDescriptionInput.value = row.querySelector('.house-description').textContent.trim();
            ...
            if (deleteBtn) {
                const name = row.querySelector('.house-name').textContent.trim();
                const rowId = row.getAttribute('data-id');

                Swal.fire({
                    icon: 'warning',
                    title: 'Delete School House?',
                    ...
                    try {
                        const response = await fetch('/api/school-houses/' + rowId, {
                            method: 'DELETE'
                        });
                        if (!response.ok && response.status !== 204) {
                            throw new Error('Failed to delete school house');
                        }
                        if (houseIdInput.value === rowId) {
                            resetForm();
                        }
                        await loadHouses();
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted',
                            text: 'School house deleted from database.',
                            timer: 1400,
                            showConfirmButton: false
                        });
                    } catch (error) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: error.message || 'Failed to delete school house.',
                            confirmButtonColor: '#8b5cf6'
                        });
                    }
                });
    ...
    if (excelBtn) {
        excelBtn.addEventListener('click', function () {
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'School Houses');
            const timestamp = new Date().toISOString().split('T')[0];
            XLSX.writeFile(wb, 'School_Houses_' + timestamp + '.xlsx');
            Swal.fire({
                icon: 'success',
                title: 'Exported!',
    ...
    if (csvBtn) {
        csvBtn.addEventListener('click', function () {
            ...
            link.setAttribute('download', 'School_Houses_' + timestamp + '.csv');
    ...
    if (pdfBtn) {
        pdfBtn.addEventListener('click', function () {
            ...
            doc.setFontSize(16);
            doc.text('Student House List', 40, 40);
            doc.setFontSize(10);
            ...
            const timestamp = new Date().toISOString().split('T')[0];
            doc.save('School_Houses_' + timestamp + '.pdf');
            Swal.fire({
                icon: 'success',
                title: 'Exported!',
    ...
    if (printBtn) {
        printBtn.addEventListener('click', function () {
            const result = getTableData();
            let printContent = ''
                + '<!DOCTYPE html><html><head><title>Student House List - Print</title><style>'
