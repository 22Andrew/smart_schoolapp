document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadLiveClasses() {
        const response = await fetch('/api/gmeet/live-classes');
        if (!response.ok) {
            const err = await response.json().catch(function () { return {}; });
    ...
    async function saveLiveClass(event) {
        ...
        if (formOptions.defaultCreatedBy) {
            payload.createdByName = formOptions.defaultCreatedBy.name;
            payload.createdByRole = formOptions.defaultCreatedBy.role;
            payload.createdById = formOptions.defaultCreatedBy.id;
        }
    ...
    async function updateStatus(id, status) {
        const response = await fetch('/api/gmeet/live-classes/' + id + '/status', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
    ...
    async function deleteLiveClass(id) {
        ...
        const response = await fetch('/api/gmeet/live-classes/' + id, { method: 'DELETE' });
        if (!response.ok) {
            const err = await response.json().catch(function () { return {}; });
    ...
    function exportTable(type) {
        ...
        const headers = ['Class Title', 'Description', 'Date Time', 'Duration', 'Created By', 'Created For', 'Class', 'Status'];
    ...
    if (tableBody) {
        tableBody.addEventListener('click', function (event) {
            ...
            if (startBtn) {
                startLiveClass(startBtn.dataset.start);
                return;
            }
